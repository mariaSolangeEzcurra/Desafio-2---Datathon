...
export default function UploadCortes({ onSincronizacionExitosa }) {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      // Enviamos el archivo junto con el proceso fijo "Corte"
    ...
    <div className="space-y-6 text-left">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
        <h3 className="text-sm font-bold text-slate-700 uppercase mb-4 flex items-center gap-2">
          <Database size={16} className="text-[#006cb7]" /> Carga de Cortes Comerciales
        </h3>
        <p className="text-xs text-slate-500 mb-4">
          Seleccione o arrastre el archivo Excel con la data de cortes para su procesamiento e inserción automática.
        </p>
        ...
        <form onSubmit={handleUpload} className="space-y-4">
          ...
          >
            {subiendo ? (
              ...
              "Iniciar Carga Masiva de Cortes"
            )}
          </button>
        </form>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
        <h3 className="text-sm font-bold text-slate-700 uppercase mb-4 flex items-center gap-2">
          <History size={16} className="text-[#006cb7]" /> Auditoría de Cargas: Cortes Comerciales
        </h3>
              ...
              {historialFiltrado.length === 0 ? (
                <tr>
                  <td colSpan={3} className="p-6 text-center text-slate-400">
                    No se registran cargas previas para el proceso de cortes.
                  </td>
                </tr>
              ) : (
